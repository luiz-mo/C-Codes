...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(opcao, "-c") == 0){
        ...
        if(gbv_create(biblioteca, key) != 0){
    ...
    if (gbv_open(&lib, biblioteca,key) != 0) {
        ...
        printf("Rode com a opcao '-c' para criar a biblioteca\n");
        return 1;
    }

    if (strcmp(opcao, "-a") == 0) {
        for (int i = 4; i < argc; i++) {
            if(gbv_add(&lib,biblioteca,argv[i],key) == 0)
        ...
        for (int i = 4; i < argc; i++) {
            ret = gbv_remove(&lib, biblioteca, argv[i],key);
            if(ret == 0)
        ...
        gbv_list(&lib, biblioteca, key);
    }
    else if (strcmp(opcao, "-v") == 0 && argc >= 5) {
        ret = gbv_view(&lib, biblioteca, argv[4],key);
        if(ret == 1)
            printf("Documento %s nao encontrado\n", argv[4]);
        else if(ret == -1)
            printf("Erro\n");
